...
void mostrar_estadisticas_totales(probador_t* probador){
  if(!probador)
    return;
  if(probador->pruebas_exitosas == probador->pruebas_totales)
    printf( COLOR_FVERDE "Total de pruebas exitosas %zu de %zu\n" COLOR_FNORMAL ,probador->pruebas_exitosas, probador->pruebas_totales);
  else
    printf( COLOR_FROJO "Total de pruebas exitosas %zu de %zu\n" COLOR_FNORMAL ,probador->pruebas_exitosas, probador->pruebas_totales);
}
